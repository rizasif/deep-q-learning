...
class Ball():
    ...
    def move(self):
        ...
        if (self.y - self.h // 2 - abs(self.vector[1]) <= self.SCOREBOARD_HEIGHT and self.vector[1] < 0) or (
                self.y + self.h // 2 + abs(self.vector[1]) >= self.GAME_AREA_RESOLUTION[1] + self.SCOREBOARD_HEIGHT and
                self.vector[1] > 0):
...
class Wimblepong(gym.core.Env):
    ...
    def __init__(self, opponent=None, visual=True):
        ...
        self.ball = Ball(self.GAME_AREA_CENTER[0], self.GAME_AREA_CENTER[1] - int(ball_size / 2), ball_size, ball_size,
                         self.GAME_AREA_RESOLUTION, self.SCOREBOARD_HEIGHT)
    ...
    def _get_observation(self, player):
        ...
        """
        This method is used to turn an RGB image into a gray scale image
        """
        def prepro(I):
